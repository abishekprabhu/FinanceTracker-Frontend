...
export class SignupComponent implements OnInit {
  ...
  passwordVisible = false; // Used to toggle password visibility
  isSpinning: boolean = false; // Indicates loading/spinner state during form submission
  ...
  onSubmit(): void {
    if (this.signupForm.valid) {
      ...
      this.authService.signup(this.signupForm.value).subscribe({
        ...
        error: (err) => {
          this.message.error(`Sign up Unsuccessful: ${err.message}`, {
            nzDuration: 5000,
          });
        },
      });
