...
export class ExpenseUpdateComponent implements OnInit{
  ...
  getExpenseById():void{
    ...
    this.expenseService.getExpenseById(this.id).subscribe({
      ...
      error:()=> this.message.error("Error while Fetching expense." , {nzDuration:5000})
    })
  }
  ...
  updateExpense(){
    ...
    // }
    this.expenseService.updateExpense(this.id,this.expenseUpdateForm.value).subscribe({
      next:(v)=>{this.message.success("Updated Successfull.",{nzDuration:5000});
      console.log(v);
      this.router.navigateByUrl("/user/expense");
    },      
      error:(err)=>this.message.error(`Error Updating expense ${err.message}` ,{nzDuration:5000})
    })
  }
