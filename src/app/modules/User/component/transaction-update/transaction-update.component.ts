...
export class TransactionUpdateComponent implements OnInit {
  ...
  getTransactionById(): void {
    ...
    this.transactionService.getTransactionById(this.id).subscribe({
      ...
      error: (err) =>
        this.message.error(`Error while Fetching transaction.${err.message}`, {
          nzDuration: 5000,
        }),
    });
  }
  ...
  updateTransaction() {
    this.transactionService
      .updateTransaction(this.id, this.transactionUpdateForm.value)
      .subscribe({
        next: (v) => {
        ...
        error: (err) =>
          this.message.error(`Error Updating Transaction ${err.message}`, {
            nzDuration: 5000,
          }),
      });
  }
