...
export class TransactionService {
  ...
  getAllTransactionByUserId(id: number): Observable<TransactionDTO[]> {
    return this.http
      .get<TransactionDTO[]>(BASE_URL + `/user/${id}`)
      .pipe(catchError(this.handleError));
  }
  ...
  updateTransaction(id: number, TransactionDTO: any): Observable<string> {
    return this.http
      .put(BASE_URL + `/${id}`, TransactionDTO, { responseType: 'text' })
      .pipe(catchError(this.handleError));
  }

  DeleteTransaction(id: number): Observable<string> {
    return this.http
      .delete(BASE_URL + `/${id}`, { responseType: 'text' })
      .pipe(catchError(this.handleError));
  }
  ...
  DownloadMonthlyReport(): Observable<any> {
    return this.http
      .get(PDF_URL + '/monthly-report', { responseType: 'blob' })
      .pipe(catchError(this.handleError));
  }
