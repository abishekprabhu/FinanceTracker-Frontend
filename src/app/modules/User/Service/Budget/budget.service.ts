...
export class BudgetService {
  ...
  createBudget(budget: any): Observable<any> {
    return this.http.post<any>(BASE_URL+`/create`, budget).pipe(
      catchError(this.handleError)
    );
  }

  getBudgetsByUser(userId: number): Observable<BudgetDTO[]> {
    return this.http.get<BudgetDTO[]>(BASE_URL+`/user/${userId}`).pipe(
      catchError(this.handleError)
    );
  }

  getBudgetsByUserAndCategory(userId: number, categoryId: number): Observable<any[]> {
    return this.http.get<any[]>(BASE_URL+`/user/${userId}/category/${categoryId}`);
  }

  getBudgetById(budgetId: number): Observable<BudgetDTO>{
    return this.http.get<BudgetDTO>(BASE_URL+`/${budgetId}`).pipe(
      catchError(this.handleError)
    )
  }
  ...
  updateBudget(budgetId:number , budgetDTO: any):Observable<BudgetDTO>{
    return this.http.put<BudgetDTO>(BASE_URL+`/${budgetId}`,budgetDTO).pipe(
      catchError(this.handleError)
    );
  }

  DeleteBudget(budgetId:number): Observable<any>{
    return this.http.delete(BASE_URL+`/${budgetId}`).pipe(
      catchError(this.handleError)
    );
  }
